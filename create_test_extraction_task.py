#!/usr/bin/env python3
"""
Create a single Sentinel-1 data extraction task for TEST split only.
...
def create_test_extraction_task(start_date="2024-09-01", end_date="2025-03-31"):
    """Create a single extraction task for the TEST split."""
    
    # Server configuration
    server_url = "http://localhost:8080"
    admin_token = "c2b46016-7893-451a-b76d-c35f495bb17b"
    
    # GeoJSON and output configuration for TEST split
    geojson_file = "croptype_KA28_wgs84_test_622.geojson"
    ...
    task_metadata = {
        ...
        "split": "test"  # Explicitly mark this as test split
    }
    
    # Task creation payload
    task_data = {
        "title": f"Sentinel-1 TEST Data Extraction {start_date} to {end_date}",
        "description": f"Extract Sentinel-1 data for TEST split from {start_date} to {end_date} using {geojson_file}",
        "assigned_to": "sentinel1-data-extraction-agent",
        "priority": "high",
        "metadata": task_metadata
    }
    
    print(f"Creating TEST extraction task...")
    print(f"Using GeoJSON: {geojson_path}")
    print(f"Using output directory: {output_dir}")
    print(f"Dataset type: testdirectory (TEST split)")
...
def main():
    parser = argparse.ArgumentParser(description="Create Sentinel-1 TEST extraction task")
    ...
    success = create_test_extraction_task(args.start_date, args.end_date)
    if not success:
        sys.exit(1)
