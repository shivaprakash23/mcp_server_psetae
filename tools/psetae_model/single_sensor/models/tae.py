...
class TemporalAttentionEncoder(nn.Module):
    def __init__(self, in_channels=128, n_head=4, d_k=32, d_model=None, n_neurons=[512, 128, 128], dropout=0.2,
        ...
        if positions == 'order':
            self.name += '_orderPos'
            # For variable sequence lengths, use len_max_seq as maximum position
            n_positions = len_max_seq
        ...
        # Create position encoding table with positions 0 to n_positions-1
        self.position_enc = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(n_positions, self.in_channels, T=T),
            freeze=True)
